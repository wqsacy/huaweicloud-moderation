<?php
	 ...
	 *  清晰度检测
	 * Created by Malcolm.
	 * Date: 2021/5/10  8:37 下午
	 */
	class Clarity
	{

		function clarity_detect ( $token , $data , $url , $threshold = 0.8 ) {
			$endPoint = get_endpoint( Constants::MODERATION );

			// 构建请求信息
			$_url = "https://" . $endPoint . Constants::IMAGE_CLARITY_DETECT;
		...
		function clarity_detect_aksk ( $_ak , $_sk , $data , $url , $threshold = 0.8 ) {
			...
			$req->scheme = "https";
			$req->host = $endPoint;
			$req->uri = Constants::IMAGE_CLARITY_DETECT;
